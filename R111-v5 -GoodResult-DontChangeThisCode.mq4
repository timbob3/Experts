...
//BUY***   
   if(  Bid > SarStop() && Close[1] > SarStop() && Bid > iMa && Bid-iMa < DistFromMa  && tradedThisBar==false)
   {
...
//SELL*****        
   if(  Bid < SarStop() && Close[1] < SarStop() && iMa-Bid < DistFromMa && Bid < iMa && tradedThisBar==false)
   {
      if(AccountFreeMargin()<(1000*Lots))
        {
         Print("We have no money. Free Margin = ", AccountFreeMargin());
