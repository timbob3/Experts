...
//    ma3 & ma4
input int    MovingPeriod3  =300;
input int    MovingPeriod4  =500;
...
int CheckForOpen()
{
  ...
  double ma1=iMA(NULL,0,MovingPeriod1,MovingShift1,MODE_EMA,PRICE_CLOSE,0);
  ...
  double ma4=iMA(NULL,TimeFrameMa3,MovingPeriod4,MovingShift4,MODE_SMA,PRICE_CLOSE,0);
...
//+------------------------------------------------------------------+  
  ...
  if(Volume[0]>1) return;
      ...
      //BUY ORDER ***   Low[1]>Low[2] && Bid>High[1]  && ma3>ma4
  if(isCrossed == 1 )  
    {         
     OrderSend(Symbol(),OP_BUY,Lots,Ask,1,Bid-StopLoss*Point,0,"",MAGICMA,0,clrAzure);
     
     // code for martyngale system
     for(int i=1; i<3; i++){OrderSend(Symbol(),OP_BUYLIMIT,Lots,Ask-i*Grid*Point,1,Bid-StopLoss*Point,0,"",MAGICMA,0,clrAzure);}    
  ...
  //for(int i=0; i<4; i++){OrderSend(Symbol(),OP_BUYLIMIT,Lots,Ask-i*Grid*Point,1,(Bid-i*Grid*Point)-StopLoss*Point,(Bid-i*Grid*Point)+TakeProfit*Point,"",MAGICMA,0,clrAzure);} // try giving each individual buy its own SL and TP.
     ...
     // SELL ORDER****  (High[1]-L>Limit*Point) && High[1]<High[2] && Bid<Low[1])  && ma3<ma4
  if(isCrossed == 2 )
    {
    OrderSend(Symbol(),OP_SELL,Lots,Bid,1,Ask+StopLoss*Point,0,"",MAGICMA,0,clrAntiqueWhite);
    
    // code for martyngale system
    for(int k=1; k<3; k++){OrderSend(Symbol(),OP_SELLLIMIT,Lots,Bid+k*Grid*Point,1,Ask+StopLoss*Point,0,"",MAGICMA,0,clrAzure);} 
