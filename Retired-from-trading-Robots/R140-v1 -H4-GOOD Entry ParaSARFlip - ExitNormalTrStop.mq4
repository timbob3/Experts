...
//BUY***   
   ...
   {
           ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,Bid-StopLoss*Point,0,"",MagicNumber,0,Blue);
           if(ticket>0)
           {
...
//SELL*****        
   ...
   {
           ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,Ask+StopLoss*Point,0,"",MagicNumber,0,Blue);
           if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) Print("BUY order opened : ",OrderOpenPrice());
            //tradedThisBar = true;
...
double SarStop()
 {
   double ParaSAR = NormalizeDouble(iSAR(0,0,SARStep,0.2,0),5);
   return(ParaSAR);   
 } 
