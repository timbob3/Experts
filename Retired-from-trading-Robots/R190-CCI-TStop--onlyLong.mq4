...
//  -----------------------    ***  RELEASED TO PRODUCTION  - 12/11/2015   ****


//                           **  .Got huge results using TS=210,SL=60, the front SarPeriod=15, and going long only..

// ----------------------------ENTRY - using 1 iSAR  + the CCI indicator.
...
//   
   double Cci1         = iCCI(NULL,0,14,PRICE_TYPICAL,0);
   double Cci2         = iCCI(NULL,0,14,PRICE_TYPICAL,1);
...
//BUY***                                      && Cci1>Cci2   && Bid>SarStop0 && Bid>SarStop2
   if(Cci1<CCILevelLow && Bid>SarStop0)// && (H-Bid)/(Point*10)>Pullback)//Bid-SarStop2>0)// && DistFromPrice < 300)//(Close[1]<SarStop() || Close[2]<SarStop() || Close[3]<SarStop()))// && tradedThisBar==false)
   {
           ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,Bid-StopLoss*Point,Ask+TakeProfit*Point," R190 ",MagicNumber,0,Blue);
           if(ticket>0)
           {
...
//SELL*****        
   //if(Cci1>CCILevelHigh && Bid<SarStop0)// && tradedThisBar==false)     && Bid<SarStop0 && Bid<SarStop2
   //{
   //        ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,Ask+StopLoss*Point,Bid-TakeProfit*Point," R190 ",MagicNumber,0,Blue);
   //        if(ticket>0)
   //        {
   //         if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) Print("BUY order opened : ",OrderOpenPrice());
   //         tradedThisBar = true;
