...
// ----------------------------ENTRY - at a set minimum distand from MA and ParaSAR flip.
...
extern int     MagicNumber    = 14083;
extern double  TakeProfit     = 20000;
extern double  TrailingStop   = 500;  //  TS
extern double  StopLoss       = 150;  //  SL
extern double  SARStep        = 0.002;
extern double  PercentRisk    = 1.0;
extern double  ManualLots     = 0;
extern double  MinDistFromMa  = 900; // distance away from MA to enter..  tweaked to 900 - test 2
extern double  PeriodMA1      =700;   //  MA1
...
//BUY***   
   if(Ma1-Bid>MinDistFromMa*Point && Bid>SarStop0)// && Close[1] >SarStop0)//&& (Close[1]<SarStop() || Close[2]<SarStop() || Close[3]<SarStop()))// && tradedThisBar==false)
   {
           ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,Bid-StopLoss*Point,0," R140-v8-c BUY ",MagicNumber,0,Blue);
           if(ticket>0)
           {
...
//SELL*****        
   if(Bid-Ma1>MinDistFromMa*Point && Bid<SarStop0)// && Close[1]<SarStop0)// && tradedThisBar==false)
   {
           ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,Ask+StopLoss*Point,0," R140-v8-c SELL ",MagicNumber,0,Blue);
           if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) Print("BUY order opened : ",OrderOpenPrice());
            tradedThisBar = true;
...
//   
//   if (Bid>SarStop0) Current_Direction = 1; // up
//   if (Bid<SarStop0) Current_Direction = 2; // down
//   
