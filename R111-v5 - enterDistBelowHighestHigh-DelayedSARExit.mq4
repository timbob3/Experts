...
//BUY***   
   if(  Bid > SarStop() && Close[1] > SarStop() && Bid > iMa && Bid-iMa > MinDistFromMa && Bid-iMa<MaxDistFromMa  && tradedThisBar==false)
   {
...
//SELL*****        
   if(  Bid < SarStop() && Close[1] < SarStop() && iMa-Bid>MinDistFromMa && iMa-Bid<MaxDistFromMa && Bid < iMa && tradedThisBar==false)
   {
      if(AccountFreeMargin()<(1000*Lots))
        {
         Print("We have no money. Free Margin = ", AccountFreeMargin());
