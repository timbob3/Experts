...
//----test code   
   ...
   //  newbar=false;
   // }
Comment("iATR = ",NormalizeDouble(iATR(NULL,0,3,0),Digits));
         ...
         if(OrderType()==OP_BUY)
           {
            if(TrailingStop>0) // && Bid-OrderOpenPrice()>Point*minMove
              {
               if(Bid-OrderOpenPrice()>AtrStop())
                 {
                  if(OrderStopLoss()<Bid-AtrStop())  // the bit that stops modifying once the price slips back..
                    {
                     OrderModify(OrderTicket(),OrderOpenPrice(),Bid-AtrStop(),OrderTakeProfit(),0,White); return(0);
                    }
                 }
         ...
         if(OrderType()==OP_SELL)
           {
            if(TrailingStop>0 )  //&& OrderOpenPrice()-Ask>Point*minMove
              ...
              if(isNewBar())
               ...
               //     {
                     OrderModify(OrderTicket(),OrderOpenPrice(),AtrStop(),OrderTakeProfit(),0,White); return(0);
                 //   }
                 //}
               }
              }
...
double AtrStop()
   {
   double ATRx3 = AtrTSFactor * NormalizeDouble(iATR(NULL,0,3,0),Digits);
   return(ATRx3);   
   } 
