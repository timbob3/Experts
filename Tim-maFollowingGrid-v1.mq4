...
input int    MovingPeriod3  =25;


input int    MovingShift1  = 0;
input int    MovingShift2  = 0;
input int    MovingShift3  = 0;
input int    MovingShift4  = 0;
input int    TimeFrameMa1  = 5;
input int    TimeFrameMa3  = 5;
...
int CheckForOpen()
{
  ...
  double ma1=iMA(NULL,TimeFrameMa1,MovingPeriod,MovingShift1,MODE_SMA,PRICE_CLOSE,0);
  double ma2=iMA(NULL,TimeFrameMa1,MovingPeriod,MovingShift2,MODE_SMA,PRICE_CLOSE,0);
  double ma3=iMA(NULL,TimeFrameMa3,MovingPeriod3,MovingShift3,MODE_LWMA,PRICE_CLOSE,0);
  //double ma3=iMA(NULL,0,MovingPeriod3,0,MODE_SMA,PRICE_CLOSE,0);
...
for(int cnt=0;cnt<OrdersTotal();cnt++)
     {
      ...
      if( OrderSymbol()==Symbol() && OrderMagicNumber()==MAGICMA)
        
        {
         if(OrderType()==OP_BUY)
           {
            if(High[1]<High[2] && Bid<Low[1] && Bid-OrderOpenPrice()>Stage1*Point)
              {
         ...
         if(OrderType()==OP_SELL)
           {
            if(Low[1]>Low[2] && Bid>High[1] && OrderOpenPrice()-Ask>Stage1*Point)
              {
