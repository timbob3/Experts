...
//--- buy conditions
   if(BoughtAlready==false && PipsFromMA1()>DistFromMA && Bid<Ma1 && Bid>High[1])//Bid<ma && ma-Bid>DistFromMA*Point)//) Crossed()==1 && 
     {
      res=OrderSend(Symbol(),OP_BUY,CalculatedLots,Ask,3,Bid-StopLoss*Point,Bid+TakeProfit*Point,"R300-v5-F2-2-BUY",MAGICMA,0,Blue);
      BoughtAlready = true;
      SoldAlready = false;
      return;
     }  
...
//--- sell conditions
   if(SoldAlready==false && PipsFromMA1()>DistFromMA && Bid>Ma1 && Ask<Low[1])//Bid>ma && Bid-ma>DistFromMA*Point)//) Crossed()==2 && 
     {
      res=OrderSend(Symbol(),OP_SELL,CalculatedLots,Bid,3,Ask+StopLoss*Point,Ask-TakeProfit*Point,"R300-v5-F2-2-SELL",MAGICMA,0,Red);
      SoldAlready = true;
      BoughtAlready = false;
      return;
     }
