...
input int    DistFromMA    = 40;
input int    PeriodMa1     = 8;
input int    PeriodMa2     = 21;
...
void CheckForOpen()
  ...
  // double Ma3 = iMA(NULL,0,PeriodMA2,0,MODE_SMA,PRICE_CLOSE,1);
   ...
   double   MyMa1 = iMA(NULL,0,PeriodMa1,0,MODE_EMA,PRICE_CLOSE,0);
   double   MyMa2 = iMA(NULL,0,PeriodMa2,0,MODE_EMA,PRICE_CLOSE,0);
...
//--- buy conditions
   if(isCrossed==1)//Bid<ma && ma-Bid>DistFromMA*Point)//) Crossed()==1 && 
     {
      res=OrderSend(Symbol(),OP_BUY,CalculatedLots,Ask,3,Bid-StopLoss*Point,Bid+TakeProfit*Point," R300-v5-copy1-BUY ",MAGICMA,0,Blue);
      BoughtAlready = true;
      SoldAlready = false;
      return;
     }  
...
//--- sell conditions
   if(isCrossed==2)//Bid>ma && Bid-ma>DistFromMA*Point)//) Crossed()==2 && 
     {
      res=OrderSend(Symbol(),OP_SELL,CalculatedLots,Bid,3,Ask+StopLoss*Point,Ask-TakeProfit*Point," R300-v5-copy1-SELL ",MAGICMA,0,Red);
      SoldAlready = true;
      BoughtAlready = false;
      return;
     }
     
   Comment("DistFromMA = ",DistFromMA,"\n"
...
//   double Ma1 = iMA(NULL,0,PeriodMA1,0,MODE_SMA,PRICE_CLOSE,0);
//  // double Ma2 = iMA(NULL,0,PeriodMA2,0,MODE_SMA,PRICE_CLOSE,0);
//   double dist = NormalizeDouble((Bid-Ma1)/(Point*10),0);  
//   
//   if(dist<0) dist = -dist;
//   return(dist);
//   }
